...
function formatWriteCall(
	...
	let text = `${theme.fg("toolTitle", theme.bold("Write"))}${theme.fg("toolTitle", `(${pathDisplay})`)}`;
	...
	if (fileContent === null) {
		...
		const body = displayLines
			.map((line, idx) => {
				const num = theme.fg("toolDiffContext", String(idx + 1).padStart(numWidth, " "));
				const prefix = idx === 0 ? BODY_JOINT : bodyIndent;
