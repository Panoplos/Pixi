...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `pixi-${platform}.zip` : `pixi-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "pixi.cmd"))) {
			writeFileSync(join(installDirectory, "pixi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\pixi.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "pixi.ps1"), '& "$PSScriptRoot/node_modules/.bin/pixi.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "pixi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\pixi.exe" %*\r\n');
		writeFileSync(join(installDirectory, "pixi.ps1"), '& "$PSScriptRoot/node_modules/.bin/pixi.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "pixi"), join(installDirectory, "pixi"));
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `pixi-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "pixi.exe" : "pixi")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "pixi.cmd" : "pixi")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "pixi.cmd" : "pixi")} --help`);
	}
}
